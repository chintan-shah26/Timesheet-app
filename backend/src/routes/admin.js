...
router.get('/timesheets', requireAdmin, async (req, res) => {
  ...
  if (month) { params.push(month); conditions.push(`TO_CHAR(t.week_start, 'YYYY-MM') = $${params.length}`); }
...
router.post('/timesheets/:id/approve', requireAdmin, async (req, res) => {
  const sheetResult = await pool.query('SELECT * FROM timesheets WHERE id = $1', [req.params.id]);
  const sheet = sheetResult.rows[0];
  if (!sheet) return res.status(404).json({ error: 'Not found' });
  if (sheet.status !== 'submitted') return res.status(400).json({ error: 'Only submitted timesheets can be approved' });
...
router.post('/timesheets/:id/reject', requireAdmin, async (req, res) => {
  ...
  const sheet = sheetResult.rows[0];
  if (!sheet) return res.status(404).json({ error: 'Not found' });
  if (sheet.status !== 'submitted') return res.status(400).json({ error: 'Only submitted timesheets can be rejected' });
...
router.get('/reports/monthly/employee', requireAdmin, async (req, res) => {
  ...
  const userResult = await pool.query("SELECT id, name, email FROM users WHERE id = $1 AND role = 'worker'", [user_id]);
  ...
  const summary = {
    ...
    total_hours: rows.reduce((s, e) => s + (e.is_present && e.hours ? parseFloat(e.hours) : 0), 0),
...
router.get('/reports/monthly/export', requireAdmin, async (req, res) => {
  ...
  if (user_id) {
    ...
    const totalHours = entries.rows.reduce((s, e) => s + (e.is_present && e.hours ? parseFloat(e.hours) : 0), 0);
    sheet.addRow({});
    sheet.addRow({ date: 'TOTAL', present: `${presentDays} days`, hours: totalHours }).font = { bold: true };

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="report-${user.name.replace(/\s+/g, '-')}-${month}.xlsx"`);
  } else {
...
router.get('/users', requireAdmin, async (req, res) => {
  const result = await pool.query("SELECT id, email, name, role, created_at FROM users ORDER BY name");
  res.json(result.rows);
});
...
router.delete('/users/:id', requireAdmin, async (req, res) => {
  ...
  const targetResult = await pool.query('SELECT id, role FROM users WHERE id = $1', [targetId]);
